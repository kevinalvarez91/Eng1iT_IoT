...
void Test_ANN_SM_Translation(ANN *net, int32_t *loc) {
    ...
    const char *motion_names[6] = {
        "Guard Stance", //motion 0
        "Strike", // motion 1
        "Sweep", // motion 2
        "Kick", //motion 3
        "Spin", // motion 4
        "Finish Pose" // motion 5
    };


    // Randomly select a motion
    int expected_motion = rand() % 6;

    printf("The program has randomly selected a motion.\n");
    printf("Please execute the following motion: %s\n", motion_names[expected_motion]);

    // LED indication for readiness
    BSP_LED_Off(LED6);
    BSP_LED_Off(LED7);
    LED_Notification_Blink_Red(3);
    LED_Notification_Blink_Green(3);
    ...
    // Run ANN to classify the motion
    run_ann(net, sig_xyz_norm);

    point = 0.0;

    // Determine the ANN's predicted motion
    ...
    // Output ANN's classification
    if (*loc >= 0 && *loc < 6) {
        printf("Neural Network Classification: %s\n", motion_names[*loc]);
    } else {
        printf("Neural Network Classification: ERROR\n");
    }

    // Compare ANN classification with expected motion
    if (*loc == expected_motion) {
        printf("Correct! The motion executed matches the prescribed motion.\n");
    } else {
        const char *detected_motion;
        if (*loc >= 0 && *loc < 6) {
            detected_motion = motion_names[*loc];
        } else {
            detected_motion = "ERROR";
        }
        printf("Incorrect. Expected motion: %s, but detected: %s\n",
               motion_names[expected_motion],
               detected_motion);
    }
